...
echo "[2b/4] Waiting for fasta file to be created"
while [ -z "$(ls $EXPECTED_FASTA 2>/dev/null)" ]; do
    sleep 10
    echo "waiting for fasta files"
done
...
while [ -z "$(ls $EXPECTED_PDB 2>/dev/null)" ]; do
    sleep 60
    echo "still waiting for output files"
done
...
echo "[4/4] Visualizing with pLDDT and PyMOL"
